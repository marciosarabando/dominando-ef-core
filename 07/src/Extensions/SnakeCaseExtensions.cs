    ...
    public static class SnakeCaseExtensions
    {
        public static void ToSnakeCaseNames(this ModelBuilder modelBuilder)
        {
            foreach (var entity in modelBuilder.Model.GetEntityTypes())
            {
                ...
                {
                    var storedObjectIdentifier = StoreObjectIdentifier.Table(tableName, null);

                    var columnName = property.GetColumnName(storedObjectIdentifier).ToSnakeCase();
                ...
                {
                    var indexName = key.GetDatabaseName().ToSnakeCase();
                    key.SetDatabaseName(indexName);
                }
            }
        }
